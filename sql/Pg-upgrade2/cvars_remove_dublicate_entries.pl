...
sub run {
  ...
  # remove all but the first one (order by itime descending)
  my $query_delete = qq|
    DELETE FROM custom_variables WHERE id = ?;
  |;
  my $sth_delete = $self->dbh->prepare($query_delete);

  my $query_all_but_last = qq|
      SELECT id FROM custom_variables WHERE trans_id = ? AND config_id = ? AND sub_module = ? ORDER BY itime DESC OFFSET 1
    |;
  my $sth_all_but_last = $self->dbh->prepare($query_all_but_last);

  foreach my $ref (@$refs) {
    my @to_delete_ids;
    $sth_all_but_last->execute($ref->{trans_id}, $ref->{config_id}, $ref->{sub_module}) || $::form->dberror($query_all_but_last);
    while (my ($row) = $sth_all_but_last->fetchrow_array()) {
      push(@to_delete_ids, $row);
    }
    ($sth_delete->execute($_) || $::form->dberror($query_delete)) for @to_delete_ids;
  }

  $sth_all_but_last->finish;
  $sth_delete->finish;
